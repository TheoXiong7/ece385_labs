...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	uint8_t send_buffer[2] = {reg + 2, val};

	if (XSpi_Transfer(&SpiInstance, send_buffer, NULL, 2) != 0) {
		print("ERROR: XSpi Transfer != 0");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for (int n=0; n < nbytes; n++) {
		uint8_t send_buffer[2] = {reg + 2, data[n]};
...
BYTE MAXreg_rd(BYTE reg) {
	...
	uint8_t send_buffer[2] = {reg, 0};
	uint8_t receive_buffer[2];

	if (XSpi_Transfer(&SpiInstance, send_buffer, receive_buffer, 2) != 0) {
		print("ERROR: XSpi Transfer != 0");
	}
